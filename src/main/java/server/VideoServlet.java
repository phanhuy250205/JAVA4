...
public class VideoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("create".equals(action)) {
            ...
            searchVideos(request, response);  // Gọi hàm tìm kiếm video
        } else {
    ...
    private void searchVideos(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            // Lọc các video có tiêu đề chứa từ khóa
            List<Videos> searchResults = videoDao.searchByTitle(keyword.trim());  // Tìm video với từ khóa
        ...
        // Chuyển hướng đến trang tìm kiếm kết quả
        request.getRequestDispatcher("/pages/videoList.jsp").include(request, response);
    }
